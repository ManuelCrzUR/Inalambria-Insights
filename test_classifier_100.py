#!/usr/bin/env python3
"""
test_classifier_100.py - Clasificar 100 plantillas frecuentes y analizar patrones
"""

import asyncio
import json
import sys
from pathlib import Path
from collections import Counter, defaultdict

sys.path.insert(0, str(Path(__file__).parent))

from config import settings
from pipeline.stages.classifier import (
    HeterogeneousPanel,
    Arbiter,
    ClassifierStage,
    ClassificationStore,
)
from pipeline.core.io_utils import iter_jsonl
from rich.console import Console
from rich.table import Table
from rich.panel import Panel

console = Console()

async def test_100_templates():
    """Clasifica 100 plantillas y analiza patrones de desacuerdo"""

    # Validar API key
    if not settings.OPENAI_API_KEY:
        console.print("[red]Error: OPENAI_API_KEY no está configurada[/red]")
        return

    console.print(Panel("[bold cyan]TEST: Clasificador con 100 Plantillas[/bold cyan]"))
    console.print(f"[cyan]Modelos configurados:[/cyan]")
    console.print(f"  • Panel 1: {settings.MODEL_PANEL_1}")
    console.print(f"  • Panel 2: {settings.MODEL_PANEL_2}")
    console.print(f"  • Arbiter: {settings.MODEL_ARBITER}")
    console.print(f"  • Threshold: 0.5\n")

    # Cargar taxonomía
    try:
        with open(settings.TAXONOMY_PATH, 'r', encoding='utf-8') as f:
            taxonomy = json.load(f)
        console.print(f"[green]✓ Taxonomía cargada: {len(taxonomy['labels'])} etiquetas[/green]\n")
    except Exception as e:
        console.print(f"[red]✗ Error cargando taxonomía: {e}[/red]")
        return

    # Leer 100 plantillas frecuentes
    templates_input = Path("output/2026-04-24/templates_with_placeholders.jsonl")

    console.print(f"[cyan]Leyendo plantillas de: {templates_input}[/cyan]")

    # Contar frecuencias primero
    templates_dict = defaultdict(int)
    count = 0
    for obj in iter_jsonl(templates_input):
        template_text = obj.get("template_text")
        if template_text:
            templates_dict[template_text] += 1
        count += 1
        if count >= 5000000:  # Limitar lectura para acelerar
            break

    # Filtrar solo frecuentes (>1) y tomar primeros 100
    frequent_templates = [
        {"template_text": t, "frequency": f}
        for t, f in sorted(templates_dict.items(), key=lambda x: x[1], reverse=True)
        if f > 1
    ][:100]

    console.print(f"[green]✓ {len(frequent_templates)} plantillas frecuentes encontradas\n[/green]")

    # Inicializar componentes
    console.print("[cyan]Inicializando Panel, Arbiter, Store...[/cyan]")
    panel = HeterogeneousPanel(api_key=settings.OPENAI_API_KEY, taxonomy_data=taxonomy)
    arbiter = Arbiter(api_key=settings.OPENAI_API_KEY, taxonomy_data=taxonomy)
    output_path = Path("output/2026-04-28/test_100_classifications.jsonl")
    output_path.parent.mkdir(parents=True, exist_ok=True)
    store = ClassificationStore(output_path)

    classifier_stage = ClassifierStage(
        panel=panel,
        arbiter=arbiter,
        store=store,
        agreement_threshold=0.5,
        concurrency=5,  # Menos concurrencia para no sobrecargar la API
    )
    console.print("[green]✓ Componentes inicializados\n[/green]")

    # Crear async iterator de plantillas
    async def template_iterator():
        for i, t_dict in enumerate(frequent_templates, 1):
            yield {
                "template_id": None,
                "template_text": t_dict["template_text"],
                "applied_rules": [],
                "client_name": "Test",
                "frequency": t_dict["frequency"],
            }

    # Estadísticas
    stats = {
        "total": 0,
        "panel_agreement": 0,
        "arbiter": 0,
        "human_review": 0,
        "errors": 0,
        "labels": defaultdict(int),
        "categories": defaultdict(int),
        "results": [],
        "disagreements": [],  # Tracking desacuerdos
    }

    console.print("[cyan]Clasificando 100 plantillas...\n[/cyan]")

    # Clasificar
    async for result in classifier_stage.classify_stream(template_iterator()):
        stats["total"] += 1
        stats["labels"][result.label] += 1
        stats["categories"][result.category] += 1
        stats["results"].append(result)

        if result.level_used == "panel_agreement":
            stats["panel_agreement"] += 1
        elif result.level_used == "arbiter":
            stats["arbiter"] += 1
            # Guardar patrón de desacuerdo
            stats["disagreements"].append({
                "template": result.template_text[:80],
                "judge_1": result.panel_judge_1,
                "judge_2": result.panel_judge_2,
                "arbiter_decision": result.label,
                "frequency": result.frequency,
            })
        elif result.level_used == "human_review":
            stats["human_review"] += 1
        elif result.level_used == "error":
            stats["errors"] += 1

        # Mostrar progreso cada 10 plantillas
        if stats["total"] % 10 == 0:
            pct = (stats["total"] / len(frequent_templates)) * 100
            console.print(f"[dim]Procesadas {stats['total']}/{len(frequent_templates)} ({pct:.0f}%)[/dim]")

    # Resumen final
    console.print("\n" + "="*70)
    console.print(Panel("[bold cyan]RESUMEN[/bold cyan]"))

    summary_table = Table(title="Estadísticas de Clasificación")
    summary_table.add_column("Métrica", style="cyan")
    summary_table.add_column("Valor", style="green")

    summary_table.add_row("Total clasificadas", str(stats["total"]))
    summary_table.add_row(
        "Panel agreement",
        f"{stats['panel_agreement']} ({100*stats['panel_agreement']/max(stats['total'],1):.1f}%)"
    )
    summary_table.add_row(
        "Arbiter resueltos",
        f"{stats['arbiter']} ({100*stats['arbiter']/max(stats['total'],1):.1f}%)"
    )
    summary_table.add_row(
        "Human review",
        f"{stats['human_review']} ({100*stats['human_review']/max(stats['total'],1):.1f}%)"
    )
    summary_table.add_row(
        "Errores",
        f"{stats['errors']} ({100*stats['errors']/max(stats['total'],1):.1f}%)"
    )

    console.print(summary_table)

    # Top categorías
    console.print("\n")
    top_categories_table = Table(title="Top Categorías")
    top_categories_table.add_column("Categoría", style="cyan")
    top_categories_table.add_column("Cantidad", style="green")
    top_categories_table.add_column("Porcentaje", style="yellow")

    for category, count in sorted(stats["categories"].items(), key=lambda x: x[1], reverse=True)[:10]:
        pct = (count / stats["total"]) * 100
        top_categories_table.add_row(category, str(count), f"{pct:.1f}%")

    console.print(top_categories_table)

    # Analizar patrones de desacuerdo
    if stats["disagreements"]:
        console.print("\n")
        console.print(Panel("[bold yellow]PATRONES DE DESACUERDO (Top 10)[/bold yellow]"))

        # Agrupar desacuerdos por patrón
        disagreement_patterns = defaultdict(int)
        for d in stats["disagreements"]:
            pattern = f"{d['judge_1']} vs {d['judge_2']}"
            disagreement_patterns[pattern] += 1

        disagreement_table = Table(title="Desacuerdos del Panel")
        disagreement_table.add_column("Patrón", style="cyan")
        disagreement_table.add_column("Ocurrencias", style="magenta")
        disagreement_table.add_column("% del Total", style="yellow")

        for pattern, count in sorted(disagreement_patterns.items(), key=lambda x: x[1], reverse=True)[:10]:
            pct = (count / len(stats["disagreements"])) * 100
            disagreement_table.add_row(pattern, str(count), f"{pct:.1f}%")

        console.print(disagreement_table)

        # Ejemplos de desacuerdos
        console.print("\n")
        console.print("[bold]Ejemplos de desacuerdos:[/bold]")
        for i, d in enumerate(stats["disagreements"][:5], 1):
            console.print(f"\n  {i}. Template: {d['template']}...")
            console.print(f"     Judge 1: {d['judge_1']}")
            console.print(f"     Judge 2: {d['judge_2']}")
            console.print(f"     Arbiter: {d['arbiter_decision']} | Freq: {d['frequency']:,}")

    # Guardar resultados
    console.print(f"\n[green]✓ Resultados guardados en: {output_path}[/green]")
    console.print(f"[green]✓ Total de líneas en JSONL: {stats['total']}[/green]")

if __name__ == "__main__":
    asyncio.run(test_100_templates())
